...
import {
  Close, Edit, AttachMoneyOutlined, PersonOutlined,
  AutorenewOutlined, CheckCircleOutlined, HourglassEmptyOutlined,
  PaymentsOutlined, SettingsOutlined, AddOutlined, SchoolOutlined,
  HistoryOutlined, ExpandMoreOutlined, ExpandLessOutlined,
...
// ─── Types ─────────────────────────────────────────────────────────────────
type Teacher = { id: number; member_id: number; full_name: string; email: string; role: string };
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
const fmt = (n: number) =>
  new Intl.NumberFormat("en-PK", { minimumFractionDigits: 0 }).format(n ?? 0);
...
function FreqChip({ freq }: { freq: string }) {
  return (
    <Chip label={freq} size="small" sx={{
      ...
      fontFamily: FONT, fontWeight: 600, fontSize: "0.7rem",
      height: 20, border: `1px solid ${C.blue}25`, textTransform: "capitalize",
    }} />
  );
}

function MonoAmt({ value, color = C.textPrimary, size = "0.82rem" }: {
...
function TeacherAvatar({ name }: { name: string }) {
  ...
  return (
    <Box sx={{
      ...
      backgroundColor: C.accentDim, border: `1px solid rgba(245,158,11,0.22)`,
      display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
    }}>
      <Typography sx={{ fontFamily: FONT, fontWeight: 700, fontSize: "0.68rem", color: C.accent }}>
        {initials}
      </Typography>
    </Box>
  );
    ...
    <Box sx={{
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 1.5 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.25 }}>
          <TeacherAvatar name={tName} />
          <Box>
            <Typography sx={{ fontWeight: 700, fontSize: "0.9rem", color: C.textPrimary, fontFamily: FONT, lineHeight: 1.2 }}>
              {tName}
            </Typography>
            <Typography sx={{ fontFamily: '"DM Mono", monospace', fontSize: "0.7rem", color: C.textSecondary }}>
      ...
      <Button size="small" onClick={onPay}
        startIcon={<PaymentsOutlined sx={{ fontSize: 13 }} />}
        sx={{
          color: C.green, fontFamily: FONT, fontWeight: 600, fontSize: "0.72rem",
          textTransform: "none", borderRadius: "7px", px: 1.5,
...
function SalaryStructureCard({ s, tName, onEdit }: { s: TeacherSalary; tName: string; onEdit: () => void }) {
  return (
    <Box sx={{
      ...
      <Typography sx={{ fontFamily: '"DM Mono", monospace', fontWeight: 700, fontSize: "1.1rem", color: C.accent, mt: 1 }}>
        PKR {fmt(s.base_salary)}
      </Typography>
...
// ─── Pay history: expandable per-teacher row ──────────────────────────────────
function HistoryTeacherRow({ teacher }: { teacher: Teacher }) {
  ...
  const toggle = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get(`/payroll/staff/${teacher.member_id}`);
    ...
    <>
        ...
        <TableCell sx={tdSx}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 0.75 }}>
            {loading
              ...
              : expanded
                ? <ExpandLessOutlined sx={{ fontSize: 16, color: C.accent }} />
                : <ExpandMoreOutlined  sx={{ fontSize: 16, color: C.textSecondary }} />
            }
      ...
      {expanded && payments.map((p, i) => (
        <TableRow key={p.id} sx={{
          ...
          animation: `fadeUp 0.22s ${i * 18}ms ease both`,
          "@keyframes fadeUp": { from: { opacity: 0, transform: "translateY(4px)" }, to: { opacity: 1, transform: "translateY(0)" } },
        }}>
          <TableCell sx={{ ...tdSx, pl: 7, fontFamily: '"DM Mono", monospace', fontSize: "0.78rem", color: C.textSecondary }}>
            {MONTHS[p.month - 1]} {p.year}
          </TableCell>
          ...
          <TableCell sx={tdSx}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              <StatusChip status={p.status} />
              {p.paid_date && (
                <Typography sx={{ fontFamily: '"DM Mono", monospace', fontSize: "0.72rem", color: C.textSecondary }}>
                  {fmtDate(p.paid_date)}
                </Typography>
              )}
            </Box>
...
export default function PayrollPage() {
  ...
  const [salaryForm,    setSalaryForm]    = useState({
    member_id: "", base_salary: "",
    pay_frequency: "monthly", effective_from: "", effective_to: "",
  });
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const allMembers: Teacher[] = teamRes.data;
      setTeachers(allMembers.filter(m => m.role !== "admin"));
  ...
  // ── Fetch history by month/year ───────────────────────────────────
  const fetchHistory = async () => {
    setHistLoading(true);
    try {
      const res = await api.get("/payroll/salaries", {
        params: { month: histMonth, year: histYear },
      });
      setAllPayments(res.data);
    } catch (err: any) {
      toast.error(err.response?.data?.detail || "Failed to load salary history");
  ...
  const handleGenerateAll = async () => {
    setGenerating(true);
    try {
      const res = await api.post("/payroll/generate-all", null, {
  ...
  const handleSaveSalary = async () => {
    if (!salaryForm.member_id || !salaryForm.base_salary || !salaryForm.effective_from)
      return toast.error("Teacher, base salary, and effective from date are required");
    setSavingSalary(true);
    try {
      const payload = {
        member_id: Number(salaryForm.member_id),
      ...
      if (editingSalary) {
        await api.put(`/payroll/staff-salary/${editingSalary.id}`, payload);
        toast.success("Salary structure updated");
      } else {
        await api.post("/payroll/staff-salary", payload);
        toast.success("Salary structure created");
      }
      setSalaryOpen(false);
      setEditingSalary(null);
      setSalaryForm({ member_id: "", base_salary: "", pay_frequency: "monthly", effective_from: "", effective_to: "" });
      await fetchAll();
    } catch (err: any) {
  ...
  const openPay = (p: SalaryPayment) => {
    setPayTarget(p);
    setPayForm({ deductions: "0", bonus: "0", payment_method: "bank_transfer" });
    setPayOpen(true);
  };

  // ── Derived ───────────────────────────────────────────────────────
  const pendingTotal = pending.reduce((s, p) => s + p.net_amount, 0);
  const tName = (memberId: number) => teachers.find(t => t.member_id === memberId)?.full_name ?? `Teacher #${memberId}`;
  ...
  return (
    <>
      ...
      <Box sx={{ p: { xs: 1.5, sm: 2.5, md: 3 }, backgroundColor: C.bg, minHeight: "100%" }}>
        ...
        <PageHeader
          title="Payroll"
          subtitle="Manage teacher salary structures and monthly payments"
          isMobile={isMobile}
        />
        ...
        <Box sx={{
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 2 }}>
            ...
            <Typography sx={{
              ...
              letterSpacing: "0.15em", textTransform: "uppercase", color: C.textSecondary,
            }}>
          ...
          <Grid container spacing={1.5} alignItems="center">
            <Grid item xs={12} sm={4}>
              <TextField select fullWidth size="small" label="Month" sx={inputSx}
                value={genMonth} onChange={e => setGenMonth(Number(e.target.value))}
            ...
            <Grid item xs={12} sm={3}>
              <TextField fullWidth size="small" label="Year" type="number" sx={inputSx}
                value={genYear} inputProps={{ min: 2020, max: 2099 }}
            ...
            <Grid item xs={12} sm={5}>
              <Button fullWidth variant="contained" onClick={handleGenerateAll} disabled={generating}
                ...
                sx={{
                  ...
                  fontFamily: FONT, fontWeight: 600, textTransform: "none", borderRadius: "10px", height: 40,
        ...
        <Box sx={{ borderBottom: `1px solid ${C.border}`, mb: 2.5 }}>
          <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{
            "& .MuiTab-root": { fontFamily: FONT, textTransform: "none", fontSize: "0.85rem", color: C.textSecondary, minHeight: 44 },
            "& .Mui-selected": { color: `${C.accent} !important`, fontWeight: 700 },
        ...
        {!loading && tab === 0 && (
          pending.length === 0 ? (
            ...
            <Box>{pending.map(p => (
              <PendingCard key={p.id} p={p} tName={tName(p.member_id)} onPay={() => openPay(p)} />
            ))}</Box>
          ) : (
            <DataTable>
              <Table>
                <TableHead>
                  <TableRow>
                    {["Teacher","Period","Gross","Deductions","Bonus","Net","Status",""].map(h => (
                ...
                <TableBody>
                  {pending.map((p, i) => (
                    <TableRow key={p.id} sx={{
                      "&:hover": { backgroundColor: "rgba(255,255,255,0.02)" }, transition: `background ${EASE}`,
                      animation: `fadeUp 0.3s ${i * 25}ms ease both`,
                      "@keyframes fadeUp": { from: { opacity: 0, transform: "translateY(6px)" }, to: { opacity: 1, transform: "translateY(0)" } },
                    }}>
                      <TableCell sx={tdSx}>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                          <TeacherAvatar name={tName(p.member_id)} />
                      ...
                      <TableCell sx={tdSx}><StatusChip status={p.status} /></TableCell>
                      <TableCell sx={tdSx}>
                        <Tooltip title="Process Payment" arrow>
                          <Button size="small" onClick={() => openPay(p)}
                            startIcon={<PaymentsOutlined sx={{ fontSize: 13 }} />}
                            sx={{
                              color: C.green, fontFamily: FONT, fontWeight: 600, fontSize: "0.72rem",
                              textTransform: "none", borderRadius: "8px", px: 1.5,
                              border: `1px solid ${C.green}30`,
                              "&:hover": { backgroundColor: C.greenDim }, transition: `all ${EASE}`,
                            }}>
                            Pay
                          </Button>
                        </Tooltip>
          ...
          <>
            <Box sx={{ display: "flex", justifyContent: "flex-end", mb: 2 }}>
              <Button variant="contained"
                ...
                onClick={() => { setEditingSalary(null); setSalaryForm({ member_id: "", base_salary: "", pay_frequency: "monthly", effective_from: "", effective_to: "" }); setSalaryOpen(true); }}
            ...
            {salaries.length === 0 ? (
              <EmptyState icon={SettingsOutlined}
                message="No salary structures yet. Add one per teacher to enable payroll generation."
                actionLabel="Add Salary Structure" onAction={() => setSalaryOpen(true)} />
            ) : isMobile ? (
              <Box>{salaries.map(s => (
                <SalaryStructureCard key={s.id} s={s} tName={tName(s.member_id)} onEdit={() => openEditSalary(s)} />
              ))}</Box>
            ) : (
              <DataTable>
                <Table>
                  <TableHead>
                    <TableRow>
                      {["Teacher","Base Salary","Frequency","Effective From","Effective To",""].map(h => (
                        <TableCell key={h} sx={thSx}>{h}</TableCell>
                      ))}
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {salaries.map((s, i) => (
                      <TableRow key={s.id} sx={{
                        ...
                        <TableCell sx={tdSx}>
                          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                            <TeacherAvatar name={tName(s.member_id)} />
                            <Typography sx={{ fontFamily: FONT, fontWeight: 700, fontSize: "0.875rem", color: C.textPrimary }}>
                              {tName(s.member_id)}
                            </Typography>
                          </Box>
                        ...
                        <TableCell sx={tdSx}>
                          <Chip label={`PKR ${fmt(s.base_salary)}`} size="small" sx={{
                        ...
                        <TableCell sx={{ ...tdSx, fontFamily: '"DM Mono", monospace', fontSize: "0.78rem", color: C.textSecondary }}>
                        ...
                        <TableCell sx={tdSx}>
                          <Tooltip title="Edit" arrow>
                            <IconButton size="small" onClick={() => openEditSalary(s)} sx={{
                              color: C.textSecondary, borderRadius: "8px", p: 0.75,
                              "&:hover": { backgroundColor: C.accentDim, color: C.accent }, transition: `all ${EASE}`,
                            }}>
                              <Edit sx={{ fontSize: 15 }} />
                            </IconButton>
                          </Tooltip>
        ...
        {!loading && tab === 2 && (
          <>
            ...
            <Box sx={{
              backgroundColor: C.surface, border: `1px solid ${C.border}`,
              borderRadius: "14px", p: { xs: 2, sm: 2.5 }, mb: 2.5,
            }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 2 }}>
              ...
              <Grid container spacing={1.5} alignItems="center">
                <Grid item xs={12} sm={4}>
                  <TextField select fullWidth size="small" label="Month" sx={inputSx}
                    value={histMonth} onChange={e => setHistMonth(Number(e.target.value))}
                    SelectProps={{ MenuProps: menuProps }}>
                    {MONTHS.map((m, i) => (
                      <MenuItem key={i} value={i + 1}>
                        <Typography sx={{ fontFamily: FONT, fontSize: "0.875rem" }}>{m}</Typography>
                ...
                <Grid item xs={12} sm={3}>
                  <TextField fullWidth size="small" label="Year" type="number" sx={inputSx}
                    value={histYear} inputProps={{ min: 2020, max: 2099 }}
                    onChange={e => setHistYear(Number(e.target.value))} />
                </Grid>
                <Grid item xs={12} sm={5}>
                  <Button fullWidth variant="contained" onClick={fetchHistory} disabled={histLoading}
            ...
            {histLoading ? (
              ...
              <Box>{allPayments.map(p => (
                <PendingCard key={p.id} p={p} tName={tName(p.member_id)} onPay={() => {}} />
              ))}</Box>
            ) : (
              <DataTable>
                <Table>
                  <TableHead>
                    <TableRow>
                      {["Teacher","Gross","Deductions","Bonus","Net","Method","Paid Date","Status"].map(h => (
                        <TableCell key={h} sx={thSx}>{h}</TableCell>
                      ))}
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {allPayments.map((p, i) => (
                      <TableRow key={p.id} sx={{
                        ...
                        <TableCell sx={tdSx}><MonoAmt value={p.bonus} color={p.bonus > 0 ? C.green : C.textSecondary} /></TableCell>
                        <TableCell sx={tdSx}><MonoAmt value={p.net_amount} color={C.accent} size="0.875rem" /></TableCell>
                        <TableCell sx={{ ...tdSx, fontFamily: FONT, fontSize: "0.78rem", color: C.textSecondary, textTransform: "capitalize" }}>
                          {p.payment_method?.replace("_", " ") ?? "—"}
                        </TableCell>
                        <TableCell sx={{ ...tdSx, fontFamily: '"DM Mono", monospace', fontSize: "0.75rem", color: C.textSecondary }}>
                          {fmtDate(p.paid_date)}
                        </TableCell>
                        <TableCell sx={tdSx}><StatusChip status={p.status} /></TableCell>
                      </TableRow>
        ...
        {/* ── Salary Structure Dialog ───────────────── */}
        <Dialog open={salaryOpen} onClose={() => setSalaryOpen(false)}
          ...
          <DialogTitle sx={{ fontFamily: FONT, fontWeight: 700, fontSize: "1.05rem", color: C.textPrimary, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              <SettingsOutlined sx={{ fontSize: 18, color: C.accent }} />
              {editingSalary ? "Edit Salary Structure" : "Add Salary Structure"}
            </Box>
            {isMobile && <IconButton onClick={() => setSalaryOpen(false)} sx={{ color: C.textSecondary }}><Close /></IconButton>}
          </DialogTitle>
          <DialogContent sx={{ pt: 2.5 }}>
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <TextField select fullWidth required label="Staff" sx={inputSx}
                  value={salaryForm.member_id}
                  onChange={e => setSalaryForm({ ...salaryForm, member_id: e.target.value })}
                  SelectProps={{ MenuProps: menuProps }} disabled={!!editingSalary}>
                  <MenuItem value="" disabled>Select Staff</MenuItem>
                  {teachers.map(t => (
                    <MenuItem key={t.id} value={t.member_id}>
                      <Box>
                        <Typography sx={{ fontFamily: FONT, fontSize: "0.875rem" }}>{t.full_name}</Typography>
                        <Typography sx={{ fontFamily: FONT, fontSize: "0.7rem", color: C.textSecondary }}>{t.email}</Typography>
                      </Box>
                    </MenuItem>
                  ))}
                </TextField>
              ...
              <Grid item xs={12}>
                <TextField fullWidth required label="Base Salary (PKR)" type="number" sx={inputSx}
                  placeholder="0" value={salaryForm.base_salary}
              ...
              <Grid item xs={12}>
                <TextField select fullWidth label="Pay Frequency" sx={inputSx}
                  value={salaryForm.pay_frequency}
                  onChange={e => setSalaryForm({ ...salaryForm, pay_frequency: e.target.value })}
                  SelectProps={{ MenuProps: menuProps }}>
                  {PAY_FREQUENCIES.map(f => (
                    <MenuItem key={f} value={f}>
                      <Typography sx={{ fontFamily: FONT, fontSize: "0.875rem", textTransform: "capitalize" }}>{f}</Typography>
                    </MenuItem>
                  ))}
                </TextField>
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth required type="date" label="Effective From" sx={inputSx}
                  value={salaryForm.effective_from} InputLabelProps={{ shrink: true }}
                  onChange={e => setSalaryForm({ ...salaryForm, effective_from: e.target.value })} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth type="date" label="Effective To" sx={inputSx}
                  value={salaryForm.effective_to} InputLabelProps={{ shrink: true }}
          ...
          <DialogActions sx={{ px: 3, pb: 2.5, gap: 1, borderTop: `1px solid ${C.border}` }}>
            <Button onClick={() => setSalaryOpen(false)} disabled={savingSalary}
              sx={{ color: C.textSecondary, fontFamily: FONT, textTransform: "none", borderRadius: "8px" }}>Cancel</Button>
            <Button variant="contained" onClick={handleSaveSalary}
              disabled={savingSalary || !salaryForm.member_id || !salaryForm.base_salary || !salaryForm.effective_from}
              ...
              {savingSalary ? "Saving…" : editingSalary ? "Save Changes" : "Add Structure"}
            </Button>
          </DialogActions>
        </Dialog>
        ...
        <Dialog open={payOpen} onClose={() => setPayOpen(false)}
          ...
          <DialogTitle sx={{ fontFamily: FONT, fontWeight: 700, fontSize: "1.05rem", color: C.textPrimary, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              <PaymentsOutlined sx={{ fontSize: 18, color: C.accent }} />
              Process Salary Payment
            </Box>
            {isMobile && <IconButton onClick={() => setPayOpen(false)} sx={{ color: C.textSecondary }}><Close /></IconButton>}
          </DialogTitle>
          <DialogContent sx={{ pt: 2.5 }}>
            {/* Info pill */}
            <Box sx={{ p: 1.75, borderRadius: "10px", backgroundColor: C.accentDim, border: `1px solid rgba(245,158,11,0.18)`, display: "flex", alignItems: "center", gap: 1.5, mb: 2.5 }}>
              <TeacherAvatar name={tName(payTarget?.member_id ?? 0)} />
              <Box>
              ...
              <Box sx={{ ml: "auto", textAlign: "right" }}>
                ...
                <Typography sx={{ fontFamily: '"DM Mono", monospace', fontSize: "0.95rem", fontWeight: 700, color: C.textPrimary }}>
                  PKR {fmt(payTarget?.gross_amount ?? 0)}
                </Typography>
              </Box>
            </Box>
            ...
            <Grid container spacing={2}>
              <Grid item xs={6}>
                <TextField fullWidth label="Deductions" type="number" sx={inputSx}
                  placeholder="0" value={payForm.deductions}
                  onChange={e => setPayForm({ ...payForm, deductions: e.target.value })}
                  InputProps={{ startAdornment: <AttachMoneyOutlined sx={{ fontSize: 15, color: C.red, mr: 0.5 }} /> }} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth label="Bonus" type="number" sx={inputSx}
                  placeholder="0" value={payForm.bonus}
                  onChange={e => setPayForm({ ...payForm, bonus: e.target.value })}
                  InputProps={{ startAdornment: <AttachMoneyOutlined sx={{ fontSize: 15, color: C.green, mr: 0.5 }} /> }} />
              </Grid>
              <Grid item xs={12}>
                <TextField select fullWidth label="Payment Method" sx={inputSx}
                  value={payForm.payment_method}
                  onChange={e => setPayForm({ ...payForm, payment_method: e.target.value })}
                  SelectProps={{ MenuProps: menuProps }}>
                  {PAYMENT_METHODS.map(m => (
                    <MenuItem key={m} value={m}>
                      <Typography sx={{ fontFamily: FONT, fontSize: "0.875rem", textTransform: "capitalize" }}>{m.replace("_", " ")}</Typography>
                    </MenuItem>
                  ))}
                </TextField>
              </Grid>
          ...
          <DialogActions sx={{ px: 3, pb: 2.5, gap: 1, borderTop: `1px solid ${C.border}` }}>
            <Button onClick={() => setPayOpen(false)} disabled={paying}
              sx={{ color: C.textSecondary, fontFamily: FONT, textTransform: "none", borderRadius: "8px" }}>Cancel</Button>
