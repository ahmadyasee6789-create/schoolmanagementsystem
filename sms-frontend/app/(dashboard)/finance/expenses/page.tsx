...
import {
  Close, Delete, Search, AttachMoneyOutlined,
...
// Category color palette (cycles through C.* tokens)
const CAT_COLORS = [
...
function ExpenseCard({ e }: { e: Expense }) {
  ...
  return (
    <Box sx={{
      backgroundColor: C.surface, border: `1px solid RS{C.border}`,
      borderRadius: "12px", p: 2, mb: 1.5,
      transition: `border-color RS{EASE}`,
      ...
      <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", mb: 1 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.25 }}>
          <Box sx={{
            ...
            backgroundColor: cc.dim, border: `1px solid RS{cc.color}25`,
        ...
        <Typography sx={{ fontFamily: '"DM Mono", monospace', fontWeight: 700, fontSize: "0.95rem", color: C.textPrimary }}>
          PKR {fmt(e.amount)}
        </Typography>
      </Box>
      <Chip
        label={e.category?.name} size="small"
        sx={{
          backgroundColor: cc.dim, color: cc.color,
          fontFamily: FONT, fontWeight: 600, fontSize: "0.68rem",
          height: 20, border: `1px solid RS{cc.color}25`,
        }}
      />
    </Box>
  );
...
// ─── Fetcher (passed to usePaginatedQuery) ──────────────────────────────
const fetchExpenses = async ({ page, limit, search, category }: {
...
export default function ExpensesPage() {
  ...
  const confirmDeleteCat = async () => {
    ...
    setDeletingCat(true);
    try {
      await api.delete(`/expenses/categories/RS{deleteCatId}`);
  ...
  // ── Summary ──────────────────────────────────────────────────────
  const totalAmount = expenses.reduce((s: number, e: Expense) => s + e.amount, 0);
    ...
    <>
      ...
      <Box sx={{ p: { xs: 1.5, sm: 2.5, md: 3 }, backgroundColor: C.bg, minHeight: "100%" }}>
        ...
        <PageHeader
          ...
          isMobile={isMobile}
          // Two action buttons — we render them manually below the header
        />
        ...
        <Box sx={{ display: "flex", gap: 1.5, mb: 2.5, flexWrap: "wrap" }}>
          ...
          <Button
            ...
            startIcon={<CategoryOutlined sx={{ fontSize: 16 }} />}
            sx={{
              color: C.textSecondary, fontFamily: FONT, fontWeight: 600,
              textTransform: "none", borderRadius: "10px", px: 2.5,
              borderColor: C.border,
              "&:hover": { borderColor: C.accent, color: C.accent, backgroundColor: C.accentDim },
            }}
          >
            Manage Categories
          </Button>
          ...
          {expenses.length > 0 && (
            <Box sx={{
              ...
              <Typography sx={{ fontFamily: '"DM Mono", monospace', fontSize: "0.82rem", fontWeight: 700, color: C.accent }}>
                PKR {fmt(totalAmount)}
              </Typography>
        ...
        <Grid container spacing={1.5} sx={{ mb: 2.5 }}>
          <Grid item xs={12} sm={7}>
            <TextField
              fullWidth size="small" placeholder="Search by description…" sx={inputSx}
              value={search}
          ...
          <Grid item xs={12} sm={5}>
            <TextField
              select fullWidth size="small" label="Category" sx={inputSx}
        ...
        {loading ? (
          <Box sx={{ display: "flex", justifyContent: "center", py: 8 }}>
            <Box sx={{
              ...
              border: `3px solid RS{C.accentDim}`, borderTopColor: C.accent,
          ...
          <Box>
            {expenses.map((e: Expense) => <ExpenseCard key={e.id} e={e} />)}
          </Box>
          ...
          <DataTable>
                  ...
                  return (
                    <TableRow
                      ...
                      sx={{
                        ...
                        transition: `background RS{EASE}`,
                        animation: `fadeUp 0.3s RS{i * 25}ms ease both`,
                        "@keyframes fadeUp": {
                    ...
                    >
                      {/* ID */}
                      <TableCell sx={{ ...tdSx, fontFamily: '"DM Mono", monospace', color: C.textSecondary, width: 70 }}>
                      ...
                      <TableCell sx={tdSx}>
                        <Chip
                          ...
                          sx={{
                            ...
                            height: 22, border: `1px solid RS{cc.color}25`,
                          }}
                        />
                      </TableCell>
                      ...
                      {/* Amount */}
                      <TableCell sx={tdSx}>
                        <Typography sx={{ fontFamily: '"DM Mono", monospace', fontWeight: 700, fontSize: "0.855rem", color: C.textPrimary }}>
                          PKR {fmt(e.amount)}
                        </Typography>
                      </TableCell>
            ...
            {/* Pagination */}
            {totalPages > 1 && (
              <Box sx={{ display: "flex", justifyContent: "center", py: 2, borderTop: `1px solid RS{C.border}` }}>
                <Pagination
                  count={totalPages} page={page}
                  onChange={(_, v) => setPage(v)}
                  sx={{
                    "& .MuiPaginationItem-root": { fontFamily: FONT, color: C.textSecondary, borderColor: C.border },
                    "& .Mui-selected": { backgroundColor: `RS{C.accent} !important`, color: "#111827 !important" },
                  }}
                />
              </Box>
            )}
        ...
        <Dialog
          ...
          PaperProps={{ sx: { backgroundColor: C.surface, border: isMobile ? "none" : `1px solid RS{C.border}`, borderRadius: isMobile ? 0 : "16px", boxShadow: "0 24px 64px rgba(0,0,0,0.5)" } }}
        >
          <DialogTitle sx={{ fontFamily: FONT, fontWeight: 700, fontSize: "1.05rem", color: C.textPrimary, borderBottom: `1px solid RS{C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              <ReceiptLongOutlined sx={{ fontSize: 18, color: C.accent }} />
              Add Expense
            </Box>
            {isMobile && <IconButton onClick={() => setExpenseOpen(false)} sx={{ color: C.textSecondary }}><Close /></IconButton>}
          </DialogTitle>
          ...
          <DialogContent sx={{ pt: 2.5 }}>
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <TextField
                  select fullWidth required label="Category" sx={inputSx}
              ...
              <Grid item xs={12}>
                <TextField
                  fullWidth label="Description" sx={inputSx}
              ...
              <Grid item xs={12}>
                <TextField
                  fullWidth required label="Amount" type="number" sx={inputSx}
                  placeholder="0"
                  value={form.amount}
          ...
          <DialogActions sx={{ px: 3, pb: 2.5, gap: 1, borderTop: `1px solid RS{C.border}` }}>
            <Button onClick={() => setExpenseOpen(false)} disabled={saving} sx={{ color: C.textSecondary, fontFamily: FONT, textTransform: "none", borderRadius: "8px" }}>
              Cancel
            </Button>
            <Button variant="contained" onClick={handleSubmit} disabled={saving || !form.category_id || !form.amount}
        ...
        >
          <DialogTitle sx={{ fontFamily: FONT, fontWeight: 700, fontSize: "1.05rem", color: C.textPrimary, borderBottom: `1px solid RS{C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              <CategoryOutlined sx={{ fontSize: 18, color: C.accent }} />
              Manage Categories
            </Box>
            {isMobile && <IconButton onClick={() => setCategoryOpen(false)} sx={{ color: C.textSecondary }}><Close /></IconButton>}
          </DialogTitle>
          ...
          <DialogContent sx={{ pt: 2.5 }}>
            ...
            <Box sx={{ display: "flex", gap: 1, mb: 3 }}>
              <TextField
                fullWidth size="small" label="New Category" sx={inputSx}
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
              {categories.length === 0 ? (
                <Typography sx={{ fontFamily: FONT, fontSize: "0.82rem", color: C.textSecondary, textAlign: "center", py: 2, fontStyle: "italic" }}>
                  No categories yet
                </Typography>
              ) : (
                categories.map(cat => {
                  ...
                  return (
                    <Box
                      ...
                      sx={{
                        ...
                        backgroundColor: C.bg, border: `1px solid RS{C.border}`,
                        transition: `border-color RS{EASE}`,
                        "&:hover": { borderColor: cc.color + "40" },
                      }}
                    >
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1.25 }}>
                        <Box sx={{ width: 10, height: 10, borderRadius: "50%", backgroundColor: cc.color, flexShrink: 0 }} />
                        <Typography sx={{ fontFamily: FONT, fontSize: "0.875rem", color: C.textPrimary, fontWeight: 500 }}>
                          {cat.name}
                        </Typography>
                      </Box>
                      <Tooltip title="Delete Category" arrow>
                        <IconButton size="small" onClick={() => setDeleteCatId(cat.id)} sx={{ color: C.textSecondary, borderRadius: "7px", p: 0.6, "&:hover": { backgroundColor: C.redDim, color: C.red }, transition: `all RS{EASE}` }}>
          ...
          <DialogActions sx={{ px: 3, pb: 2.5, borderTop: `1px solid RS{C.border}` }}>
            <Button onClick={() => setCategoryOpen(false)} sx={{ color: C.textSecondary, fontFamily: FONT, textTransform: "none", borderRadius: "8px" }}>
              Close
            </Button>
          </DialogActions>
        </Dialog>
