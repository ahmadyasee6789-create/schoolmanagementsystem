...
export default function DeleteTermDialog({ open, onClose, onConfirm, loading }: DeleteTermDialogProps) {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
      <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-5 shadow-xl dark:border-white/10 dark:bg-[#1a2233]">
        <div className="mb-3 flex items-center gap-3">
          ...
          <h2 className="text-base font-bold text-slate-900 dark:text-slate-50">Delete Term?</h2>
        </div>
        <p className="mb-5 text-sm text-slate-500 dark:text-slate-400">
          This will permanently delete the term. Deletion is blocked if exams are linked to it.
        </p>
